...
: 시스템 부팅 시 자동으로 마운트 될 파일시스템을 기록
    ...
    * 구성
    장치명 마운트포인트 파일시스템종류 마운트옵션 점검여부 점검우선순위
...
![img]({{ '/assets/images/2022/11/02/img4.PNG' | relative_url }}){: .left-image }*데몬구동, 확인, 정지*
...
리눅스 방화벽에 등록이 안된 경우
루트라 할지라도 selinux(security linux)에 등록해야 실행가능
=> 둘다 비활성화 할 것이다.
...
![img]({{ '/assets/images/2022/11/02/img7.PNG' | relative_url }}){: .left-image }*현재 구성한 네트워크*
