...
* searchsorted: 정렬된 array에서 요소를 찾기
...
```python
arr1 = a[3:8] # 값을 복사하는 것이 아니라 참조를 하고 있기에 이 array를 변경하면 원 array도 바뀐다!(얕은 복사)
arr1
```
...
* array과 단일 숫자(scalar) 사이의 연산(벡터와 스칼라 간의 연산이라고도 함) 또는 두 개의 서로 다른 크기의 array 간에 연산을 수행
...
* 두 개의 array에서 작업할 때 NumPy는 요소별로 모양을 비교한다. 후행(즉, 맨 오른쪽) 차원에서 시작하여 왼쪽으로 작동한다.
